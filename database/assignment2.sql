...
-- Changing Stark's `account_type`
UPDATE public.account
SET account_type = 'Admin'
WHERE account_id = 1;

-- Deleting Stark from `account`
DELETE FROM public.account
WHERE account_id = 1;

-- Updating `description` on GM Hummer
UPDATE public.inventory
SET inv_description = REPLACE(inv_description, 'small interiors', 'a huge interior')
WHERE inv_id = 10;

-- Inner join with `inventory` sports
SELECT
    inv_make,
    inv_model,
    classification_name
...
-- Corecting file path in `inventory`
UPDATE public.inventory
